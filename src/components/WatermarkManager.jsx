...
export default function WatermarkManager({ apiUrl }) {
  ...
  const loadWatermarks = async () => {
    setLoading(true)
    try {
      const response = await fetch(`${apiUrl}/assets/watermarks`)
      const data = await response.json()
      setWatermarks(data.watermarks || [])
    } catch (error) {
      console.error('Failed to load watermarks:', error)
      setMessage({ type: 'error', text: 'Failed to load watermarks' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const loadGroups = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/assets/watermarks/groups`)
      const data = await response.json()
  ...
  const handleUpload = async (event) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/assets/watermarks/upload`, {
        method: 'POST',
        body: formData
      })
  ...
  const handleDelete = async (filename) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/assets/watermarks/${encodeURIComponent(filename)}`, {
        method: 'DELETE'
      })
  ...
  const handleDeleteGroup = async (groupId, groupName) => {
    if (!confirm(`Delete group "${groupName}"? This will remove all watermarks from the group.`)) return

    try {
      const response = await fetch(`${apiUrl}/assets/watermarks/groups/${groupId}`, {
        method: 'DELETE'
      })
  ...
  const handleAddToGroup = async (watermarkFilename, groupId) => {
    try {
      const response = await fetch(`${apiUrl}/assets/watermarks/groups/${groupId}/members`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ watermark_filename: watermarkFilename })
      })

      if (response.ok) {
        setMessage({ type: 'success', text: `Added "${watermarkFilename}" to group` })
        await loadGroups()
      } else {
        const error = await response.json()
  ...
  const handleRemoveFromGroup = async (watermarkFilename, groupId) => {
    try {
      const response = await fetch(`${apiUrl}/assets/watermarks/groups/${groupId}/members/${encodeURIComponent(watermarkFilename)}`, {
        method: 'DELETE'
      })

      if (response.ok) {
        setMessage({ type: 'success', text: `Removed "${watermarkFilename}" from group` })
        await loadGroups()
      } else {
        const error = await response.json()
  ...
  const getGroupsForWatermark = (filename) =>
    groups.filter(group => group.members?.some(m => m.watermark_filename === filename))
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <label className="btn btn-primary" style={{ cursor: 'pointer', position: 'relative' }}>
            ...
            <input
              type="file"
              accept=".mp4,.mov,.avi,.mkv,.png,.jpg,.jpeg,.webp"
        ...
        <div className="card" style={{ marginBottom: '24px' }}>
          ...
          {groupsLoading ? (
              ...
              {groups.map(group => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
                    <div>
                      <h4 style={{ margin: 0, color: group.color || '#38bdf8' }}>{group.name}</h4>
                      {group.description && (
            ...
            <div className="empty-state" style={{ padding: '20px' }}>
              ...
              <p>Create a group to organize your watermark files</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="stats-grid" style={{ marginBottom: '24px' }}>
        <div className="stat-card" style={{ background: 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)' }}>
          <div className="stat-value">{watermarks.length}</div>
          <div className="stat-label">Total Watermark Files</div>
        </div>
        <div className="stat-card" style={{ background: 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)' }}>
          <div className="stat-value">
            {watermarks.reduce((sum, w) => sum + w.size_mb, 0).toFixed(2)} MB
          </div>
      ...
      <div className="card">
        {loading ? (
            ...
            <table>
              ...
              <tbody>
                {watermarks.map((watermark, index) => {
                  const assignedGroups = getGroupsForWatermark(watermark.name)
                  return (
                    <tr key={index}>
                      <td>
                      ...
                      <td>{watermark.size_mb} MB</td>
                      <td>{formatDate(watermark.modified)}</td>
                      <td>
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                          {groups.length > 0 && (
                            <select
                              onChange={(e) => {
                                if (e.target.value) {
                                  handleAddToGroup(watermark.name, e.target.value)
                                  e.target.value = ''
                                }
                              }}
                              style={{
                            ...
                            <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                              {assignedGroups.map(group => (
                                ...
                                >
                                  {group.name}
                                  <button
                                    onClick={() => handleRemoveFromGroup(watermark.name, group.id)}
                                    style={{
                          ...
                          <button
                            className="btn btn-danger"
                            onClick={() => handleDelete(watermark.name)}
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-text">No watermark files</div>
            <p>Upload watermark files (video or image) to get started</p>
          </div>
        )}
      </div>
    </div>
